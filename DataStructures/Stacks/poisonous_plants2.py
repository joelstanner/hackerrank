...
def check_pesticide_lvl(plants):
    """Traverse the list to find each successive next lower number

    Every time a new lower is found, a new day is added.
    """
    days = 0
    pointer = 0
    while True:
        try:
            next_idx = next(i for i, v in enumerate(plants, pointer)
                            if v < plants[pointer])
            days += (next_idx - pointer - 1)
            pointer = next_idx
...
print(check_pesticide_lvl(PLANTS))
