...
In the second case (when N = 1, i.e. after the 1st cycle), the tree doubles
its height as it's planted at the onset of spring.

In the third case (N=4), the tree first doubles its height (2), then grows a
meter (3), then doubles again (6), before growing another meter; at the end of
the 4th cycle, its height is 7 meters.
...
def utopian_tree_growth(cycles):
    """Return the height of the tree from the number of growth cycles."""
    heigth = 0
    while cycles >= 0:
        heigth += 1
        cycles -= 1
...
for _ in range(T):
    print(utopian_tree_growth(int(input())))
